...
// Automatically rotate about y axis
...
float alpha1 = 0.03f, alpha2 = 0.01f; // compute rail cross-section vertices
...
std::vector<glm::vec3> leftRail_N, sleeper_N, rightRail_N; // rail triangle vertex normals
...
std::vector<glm::vec3> leftSupport_N, rightSupport_N; // rail support triangle normals
...
std::vector<glm::vec3> skyboxLPos, skyboxRPos, skyboxFPos, skyboxBPos, skyboxDPos, skyboxTPos; // skybox pos
std::vector<glm::vec2> skyboxLUV, skyboxRUV, skyboxFUV, skyboxBUV, skyboxDUV, skyboxTUV; //skybox uv
GLuint vboSkyboxL, vboSkyboxR, vboSkyboxF, vboSkyboxB, vboSkyboxD, vboSkyboxT; // skybox vbos
GLuint vaoSkyboxL, vaoSkyboxR, vaoSkyboxF, vaoSkyboxB, vaoSkyboxD, vaoSkyboxT; // skybox vao
GLuint texHandleL, texHandleR, texHandleF, texHandleB, texHandleD, texHandleT; // texture handles

std::vector<glm::vec3> OBJvertices; // object vertices
std::vector<glm::vec3> OBJnormals; // object normals
OBJloader myOBJ; // Trex
GLuint vboOBJvertices, vboOBJnormals, vaoOBJ; // vbos and vaos for object
...
{
	glActiveTexture(unit); // select active texture
	// get handle on textureImage
	GLint h_textureImage = glGetUniformLocation(texturePipelineProgram->GetProgramHandle(), "textureImage");
	// read from texture unit
	glUniform1i(h_textureImage, unit - GL_TEXTURE0);
}

void passPhongShadingForSkybox() // pass values to shaders for skybox
{
	// for skybox texture
	float La[4] = { 0.15f, 0.45f, 0.45f }; float ka[4] = { 0.8f, 0.8f, 0.8f };
	...
	float Ld[4] = { 0.1f, 0.7f, 0.7f }; float kd[4] = { 0.9f, 0.9f, 0.9f };
	...
	float Ls[4] = { 0.0f, 0.75f, 0.75f }; float ks[4] = { 1.0f, 1.0f, 1.0f };
	...
	glm::vec3 lightDirection = glm::vec3(0.0f, 1.0f, 0.0f); // noon
	float viewLightDirection[3]; // light direction in view space
	...
	// upload viewLightDirection to GPU
...
void passPhongShadingforOBJ() // pass values to shader for Trex
{
	// Green metal TRex
	float LaOBJ[4] = { 0.5f,0.5f,0.5f }; float kaOBJ[4] = { 0.25f, 0.5f, 0.0f };
	...
	float LdOBJ[4] = { 0.7f, 0.7f, 0.7f }; float kdOBJ[4] = { 0.35f, 0.7f, 0.1f };
	...
	float LsOBJ[4] = { 0.9f, 0.9f, 0.9f }; float ksOBJ[4] = { 0.3f, 0.9f, 0.0f };
	...
	glm::vec3 lightDirectionOBJ = glm::vec3(0.0f, 1.0f, 0.0f); // noon
	float viewLightDirectionOBJ[3]; // light direction in view space
...
void displayFunc()
{
	...
	if (!autoRot) // if rotate is toggled on, dont ride track
	{
...
{
	// do some stuff... 
	// control speed of ride
	if (camAt < splinePoints.size()-1)
		camAt += 1;
	else
		camAt = 0;
...
void reshapeFunc(int w, int h)
{
	...
	matrix.Perspective(60.0f, (float)w / (float)h, 0.01f, 2000.0f);
}
...
void keyboardFunc(unsigned char key, int x, int y)
{
	...
	case 'r':
		// rotate automatically about y axis, toggle to go back to coaster
...
int initTexture(const char* imageFilename, GLuint textureHandle)
{
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 0.5 * fLargest);
...
// get spline points, tangents, normals, and binormals
void getPointsAndTNB()
{
	...
	for (int i = 0; i < numSplines; i++)
	{
		...
		// For first spline point : N0 = unit(T0 x V) and B0 = unit(T0 x N0)
		// For others: N1 = unit(B0 x T1) and B1 = unit(T1 x N1).
...
// hollow rail tracks
// rails are above the other
// sleeper runs between top face of left and bottom face of right
...
void initVBO_VAO_RailsAndSupports()
{
	...
	glGenBuffers(1, &vboLeftRail_N);
	glBindBuffer(GL_ARRAY_BUFFER, vboLeftRail_N);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * leftRail_N.size(), leftRail_N.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboSleeper);
	glBindBuffer(GL_ARRAY_BUFFER, vboSleeper);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * sleeper.size(), sleeper.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboSleeper_N);
	glBindBuffer(GL_ARRAY_BUFFER, vboSleeper_N);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * sleeper_N.size(), sleeper_N.data(), GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboRightRail_N);
	glBindBuffer(GL_ARRAY_BUFFER, vboRightRail_N);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * rightRail_N.size(), rightRail_N.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboLeftSupport);
	glBindBuffer(GL_ARRAY_BUFFER, vboLeftSupport);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * leftSupport.size(), leftSupport.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboLeftSupport_N);
	glBindBuffer(GL_ARRAY_BUFFER, vboLeftSupport_N);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * leftSupport_N.size(), leftSupport_N.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboRightSupport);
	glBindBuffer(GL_ARRAY_BUFFER, vboRightSupport);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * rightSupport.size(), rightSupport.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboRightSupport_N);
	glBindBuffer(GL_ARRAY_BUFFER, vboRightSupport_N);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * rightSupport_N.size(), rightSupport_N.data(), GL_STATIC_DRAW);
...
{
	// VBOs
	glGenBuffers(1, &vboSkyboxL);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxL);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxLPos.size() + sizeof(glm::vec2) * skyboxLUV.size(), NULL, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboSkyboxR);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxR);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxRPos.size() + sizeof(glm::vec2) * skyboxRUV.size(), NULL, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboSkyboxF);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxF);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxFPos.size() + sizeof(glm::vec2) * skyboxFUV.size(), NULL, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboSkyboxB);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxB);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxBPos.size() + sizeof(glm::vec2) * skyboxBUV.size(), NULL, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboSkyboxD);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxD);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxDPos.size() + sizeof(glm::vec2) * skyboxDUV.size(), NULL, GL_STATIC_DRAW);
	...
	glGenBuffers(1, &vboSkyboxT);
	glBindBuffer(GL_ARRAY_BUFFER, vboSkyboxT);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * skyboxTPos.size() + sizeof(glm::vec2) * skyboxTUV.size(), NULL, GL_STATIC_DRAW);
...
void initVBO_VAO_OBJ()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * OBJvertices.size(), OBJvertices.data(), GL_STATIC_DRAW);

	glGenBuffers(1, &vboOBJnormals);
	glBindBuffer(GL_ARRAY_BUFFER, vboOBJnormals);

	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * OBJnormals.size(), OBJnormals.data(), GL_STATIC_DRAW);
	...
	GLuint loc = glGetAttribLocation(OBJPipelineProgram->GetProgramHandle(), "position");
	glEnableVertexAttribArray(loc);
	const void* offset = (const void*)0;
	glVertexAttribPointer(loc, 3, GL_FLOAT, GL_FALSE, 0, offset);
	...
	loc = glGetAttribLocation(OBJPipelineProgram->GetProgramHandle(), "normal");
	glEnableVertexAttribArray(loc);
	offset = (const void*)0;
	glVertexAttribPointer(loc, 3, GL_FLOAT, GL_FALSE, 0, offset);
}
...
void initScene(int argc, char* argv[])
{
	...
	bool res = myOBJ.readOBJfile("objects/tyra.obj", OBJvertices, OBJnormals);
